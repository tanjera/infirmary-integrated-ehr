            ...
            <div class="flex justify-end">
                @if(Auth::user()->isManager())
                    <td class="align-content-center text-sm">
                        <a href="/chart/diagnostics/delete/{{ $report->id }}" class="btn btn-outline-danger px-3 py-1 text-sm">Delete Report</a>
                    </td>
                @endif

                @if(Auth::user()->canChart())
                    <a href="/chart/diagnostics/append/{{ $report->id }}" class="btn btn-outline-success px-3 py-1 ms-2 text-sm">Append an Addition</a>
                @endif
            </div>
        </div>
    @endsection
